...
GITHUB_TOKEN = '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'
ADMIN_ID = "6779617599"
...
async def check_session_url(session_url):
    ...
    try:
        if "sessionId" in session_url or "gw_id" in session_url or "gw_sn" in session_url:
            return True
            
        async with session.get(session_url, allow_redirects=True, headers=headers) as response:
            text_ = str(response.url)
            if "sessionId" in text_ or "gw_id" in text_ or response.status == 200:
                return True
            else:
                return False
    except Exception as e:
